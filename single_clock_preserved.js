...
function startPause(start_id,count){
    ...
    document.getElementById(start_id).innerHTML="Stop";            //no matter what button is pressed, it should have 'Stop'
  ...
  xmlHttp.onreadystatechange = function(){                                                 //shoot when ready
      if(xmlHttp.readyState == 4 && xmlHttp.status == 200)
          ...
          if(run_button==proj_id){                                                        //if same project is selected (or stopped)
              document.getElementById(run_button).innerHTML="Start";                      //restore it to original state
